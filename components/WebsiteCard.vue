<template>
  <div
    class="flex flex-wrap w-full md:w-1/2 xl:w-1/3 max-w-full"
    :class="{ 'animate-wiggle': editDashboard }"
  >
    <div class="w-full m-4">
      <nuxt-link
        class="relative group"
        :to="props.website.url"
        @click.prevent="linkClicked"
      >
        <button
          v-if="editDashboard"
          class="rounded-full p-2 absolute -top-3 -right-3 z-20 bg-black opacity-80 focus:outline-none"
        >
          <XMarkIcon
            class="w-4 h-4 text-white opacity-100 hover:text-lime-500"
            @click.prevent.stop="showRemoveWebsiteModal = true"
          />
        </button>
        <div class="rounded-xl w-full overflow-hidden">
          ...
          <div
            ...
            class="rounded-xl flex items-center justify-center w-full h-56 bg-gray-300 dark:bg-stone-700 text-gray-200 dark:text-stone-600 animate-pulse"
          >
            <svg
              class="w-12 h-12"
  ...
  <RemoveWebsiteModal
    :open="showRemoveWebsiteModal"
    @close="showRemoveWebsiteModal = false"
    @confirm="deleteWebsite"
  />
  <WebsiteModal
    :open="showEditWebsiteModal"
    :website="props.website"
    @close="showEditWebsiteModal = false"
    @confirm="editWebsite"
  />
</template>
...
function editWebsite(data: Partial<Website>) {
